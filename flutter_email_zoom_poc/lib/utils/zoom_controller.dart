...
class ZoomController extends ChangeNotifier {
  ...
  static const double minMessageScale = 0.75;
  static const double maxMessageScale = 3.0;
  ...
  double effectiveFontSizeForMessage(String messageId) {
    return effectiveFontSize * (getMessageScale(messageId) ?? 1.0);
  }
  ...
  void setMessageScale(String messageId, double scale) {
    final clamped = scale.clamp(minMessageScale, maxMessageScale);
    final current = _messageScales[messageId];
    if (current != null && (current - clamped).abs() < 0.01) {
      return;
    }
    _messageScales[messageId] = clamped;
    notifyListeners();
  }
